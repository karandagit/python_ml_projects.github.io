...
class Bill_App:
    def __init__(self,root):
        ...
        cname_bl = Label(F1, text="Customer Name", bg=bg_color, fg="white", font=("times new roman",18,"bold")).grid(row=0,column=0,padx=20,pady=5)
        cname_txt = Entry(F1, width=15, textvariable=self.c_name, font="arial 15", bd=7, relief=SUNKEN).grid(row=0,column=1,pady=5,padx=10)

        cphn_bl = Label(F1, text="Phone Number", bg=bg_color, fg="white", font=("times new roman", 18, "bold")).grid(row=0, column=2, padx=20, pady=5)
        cphn_txt = Entry(F1, width=15, textvariable=self.c_phone, font="arial 15", bd=7, relief=SUNKEN).grid(row=0, column=3, pady=5, padx=10)

        cbill_bl = Label(F1, text="Bill Number", bg=bg_color, fg="white", font=("times new roman", 18, "bold")).grid(row=0, column=4, padx=20, pady=5)
        ...
        g2_lbl = Entry(F3, width=10, textvariable=self.food_oil, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=1,column=1,padx=10,pady=10)
        ...
        c2_lbl = Entry(F4, width=10, textvariable=self.coke, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=1, column=1,padx=10, pady=10)
        ...
        F5.place(x=1050, y=180, width=400, height=380)
        ...
        t1_lbl = Label(F6, text="Cosmetic Tax", bg=bg_color, fg="white",font=("times new roman", 14, "bold")).grid(row=0, column=2, padx=20, pady=1, sticky="w")
        ...
        t2_lbl = Label(F6, text="Grocery tax", bg=bg_color, fg="white",font=("times new roman", 14, "bold")).grid(row=1, column=2, padx=20, pady=40, sticky="w")
        t2_txt = Entry(F6, width=18, textvariable=self.grocery_tax, font="arial 10 bold", bd=7, relief=SUNKEN).grid(row=1, column=3, padx=10, pady=40)

        t3_lbl = Label(F6, text="Cold Drinks Tax", bg=bg_color, fg="white",font=("times new roman", 14, "bold")).grid(row=2, column=2, padx=20, pady=10, sticky="w")
        t3_txt = Entry(F6, width=18, textvariable=self.cold_drink_tax, font="arial 10 bold", bd=7, relief=SUNKEN).grid(row=2, column=3, padx=10, pady=10)

        btn_F = Frame(F6, bd=7,relief=GROOVE)
        btn_F.place(x=850,width=585,height=155)

        total_btn = Button(btn_F, command=self.total, text="Total", bg="cadetblue", fg="white", bd=5, pady=25, width=11, font="arial 12 bold").grid(row=0, column=0, padx=5, pady=5)
        gBill_btn = Button(btn_F, text="Generate Bill", command=self.bill_area, bg="cadetblue", fg="white", bd=5, pady=25, width=11,font="arial 12 bold").grid(row=0, column=1, padx=5, pady=5)
        clear_btn = Button(btn_F, text="Clear", command=self.clear_data, bg="cadetblue", fg="white", bd=5, pady=25, width=11,font="arial 12 bold").grid(row=0, column=2, padx=5, pady=5)
        exit_btn = Button(btn_F, text="Exit", command=self.exit_bill, bg="cadetblue", fg="white", bd=5, pady=25, width=11,font="arial 12 bold").grid(row=0, column=3, padx=5, pady=5)
        self.welcome_bill()
    ...
    def total(self):
        ...
        self.c_d_tax = round((self.total_cold_drink_price * 0.05),2)
    ...
    def bill_area(self):
        if self.c_name.get() == "" or self.c_phone.get() == "":
            ...
            messagebox.showerror("Error", "None of the product has been purchased")
        else:
            self.welcome_bill()
            if self.soap.get() != 0:
            ...
            if self.lotion.get() != 0:
                self.txtarea.insert(END, f"\n Bath lotion\t\t{self.lotion.get()}\t\t{self.c_bl_p}")
            ...
            if self.thumsup.get() != 0:
                self.txtarea.insert(END, f"\n Thumps Up\t\t{self.thumsup.get()}\t\t{self.d_t_p}")

            if self.limca.get() != 0:
                self.txtarea.insert(END, f"\n Limca\t\t{self.limca.get()}\t\t{self.d_l_p}")

            if self.sprite.get() != 0:
                self.txtarea.insert(END, f"\n Sprite\t\t{self.spray.get()}\t\t{self.d_s_p}")
    ...
    def clear_data(self):
        op = messagebox.askyesno("Clear", "Do you really want to clear bill data?")
        if op > 0:
            self.soap.set(0)
